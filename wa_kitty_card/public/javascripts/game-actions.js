(function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    window.updateGrid = function(gridData) {
      gridData.forEach(cell => {
        ...
        if (gridItem) {
          ...
          if (cell.card !== 'Empty') {
            gridItem.classList.add('card-placed');
            if (wasPlaced && existingPlayer) {
              gridItem.setAttribute('data-player', existingPlayer);
            }
          } else {
            gridItem.classList.remove('card-placed');
    ...
    window.updateHand = function(hand, forPlayerNumber) {
      ...
      // forPlayerNumber === null means "force update" (from broadcast)
      if (forPlayerNumber !== null && forPlayerNumber && myPlayerNumber && forPlayerNumber !== myPlayerNumber) {
        return;
      }
